# Story Index

This is the canonical backlog for implementation stories.

## Backlog

| ID | Title | Status | Priority | File |
|---|---|---|---|---|
| STORY-001 | Enforce pipeline artifact sync | done | P1 | [STORY-001](STORY-001-pipeline-artifact-sync.md) |
| STORY-002 | Enforce generator output contract | done | P1 | [STORY-002](STORY-002-generator-output-contract.md) |
| STORY-003 | Harden normalization and dedup rules | done | P1 | [STORY-003](STORY-003-normalization-and-dedup.md) |
| STORY-004 | Fix hyphen decoding node-split bug | done | P1 | [STORY-004](STORY-004-hyphen-decoding-node-split.md) |
| STORY-005 | Add graph-cycle policy and safeguards | done | P2 | [STORY-005](STORY-005-graph-cycle-policy.md) |
| STORY-006 | Add concept-pipeline quality report | done | P2 | [STORY-006](STORY-006-quality-report.md) |
| STORY-007 | Add regression tests for pipeline and graph build | done | P1 | [STORY-007](STORY-007-regression-tests.md) |
| STORY-008 | Replace global prompt exclude with local exclude + runtime dedup | done | P1 | [STORY-008](STORY-008-local-exclude-hybrid-dedup.md) |
| STORY-009 | Add two-phase coverage generation strategy | done | P1 | [STORY-009](STORY-009-two-phase-coverage-generation.md) |
| STORY-010 | Add unexplored frontier detection tool | done | P1 | [STORY-010](STORY-010-unexplored-frontier-tool.md) |
| STORY-011 | Add auth and persistence foundation | done | P1 | [STORY-011](STORY-011-auth-foundation.md) |
| STORY-012 | Add account registration flow | done | P1 | [STORY-012](STORY-012-account-registration.md) |
| STORY-013 | Add login logout and session lifecycle | done | P1 | [STORY-013](STORY-013-login-logout-session.md) |
| STORY-014 | Add change password flow | done | P1 | [STORY-014](STORY-014-change-password.md) |
| STORY-015 | Persist user exploration progress | draft | P1 | [STORY-015](STORY-015-user-progress-persistence.md) |
| STORY-016 | Add auth and progress regression test coverage | draft | P2 | [STORY-016](STORY-016-auth-progress-regression-tests.md) |
| STORY-017 | Tighten repository hygiene and bootstrap contracts | done | P1 | [STORY-017](STORY-017-repo-hygiene-and-bootstrap.md) |
| STORY-018 | Separate runtime artifacts from versioned data fixtures | done | P1 | [STORY-018](STORY-018-runtime-artifact-boundaries.md) |
| STORY-019 | Extract shared brain utilities and split generator responsibilities | done | P1 | [STORY-019](STORY-019-brain-modularization.md) |
| STORY-020 | Decompose oversized concept-graph frontend modules | done | P2 | [STORY-020](STORY-020-graph-frontend-decomposition.md) |
| STORY-021 | Align active documentation with current implementation and cleanup rules | done | P2 | [STORY-021](STORY-021-docs-architecture-sync.md) |
| STORY-022 | Add brain CLI entry point and command registry | done | P1 | [STORY-022](STORY-022-brain-cli-entrypoint-and-command-registry.md) |
| STORY-023 | Add brain REPL shell core | done | P1 | [STORY-023](STORY-023-brain-repl-shell-core.md) |
| STORY-024 | Add brain help command | done | P2 | [STORY-024](STORY-024-brain-help-command.md) |
| STORY-025 | Add brain exit command | done | P1 | [STORY-025](STORY-025-brain-exit-command.md) |
| STORY-026 | Add brain status command | done | P1 | [STORY-026](STORY-026-brain-status-command.md) |
| STORY-027 | Add brain doctor command | done | P1 | [STORY-027](STORY-027-brain-doctor-command.md) |
| STORY-028 | Add brain load command | done | P1 | [STORY-028](STORY-028-brain-load-command.md) |
| STORY-029 | Add brain current command | ready | P2 | [STORY-029](STORY-029-brain-current-command.md) |
| STORY-030 | Add brain search command | ready | P1 | [STORY-030](STORY-030-brain-search-command.md) |
| STORY-031 | Add brain use command | ready | P1 | [STORY-031](STORY-031-brain-use-command.md) |
| STORY-032 | Add brain show command | ready | P1 | [STORY-032](STORY-032-brain-show-command.md) |
| STORY-033 | Add brain children command | ready | P2 | [STORY-033](STORY-033-brain-children-command.md) |
| STORY-034 | Add brain parents command | ready | P2 | [STORY-034](STORY-034-brain-parents-command.md) |
| STORY-035 | Add brain neighbors command | ready | P2 | [STORY-035](STORY-035-brain-neighbors-command.md) |
| STORY-036 | Add brain path command | ready | P2 | [STORY-036](STORY-036-brain-path-command.md) |
| STORY-037 | Add brain prompt command | ready | P2 | [STORY-037](STORY-037-brain-prompt-command.md) |
| STORY-038 | Add brain generate start command | ready | P1 | [STORY-038](STORY-038-brain-generate-start-command.md) |
| STORY-039 | Add brain generate resume command | ready | P1 | [STORY-039](STORY-039-brain-generate-resume-command.md) |
| STORY-040 | Add brain sync command | ready | P1 | [STORY-040](STORY-040-brain-sync-command.md) |
| STORY-041 | Add brain quality report command | ready | P1 | [STORY-041](STORY-041-brain-quality-report-command.md) |
| STORY-042 | Add brain frontier command | ready | P1 | [STORY-042](STORY-042-brain-frontier-command.md) |
| STORY-043 | Add brain coverage plan command | ready | P2 | [STORY-043](STORY-043-brain-coverage-plan-command.md) |
